...
# DEMO SEEDER
# ---------------------------------------------------------------------------
# Runs once on startup. Creates a full imaginary institute for pitching so the
# app never looks empty on a fresh database. Idempotent — does nothing if the
# demo owner already exists.
# ---------------------------------------------------------------------------

DEMO_OWNER_EMAIL = "demo@ibex.in"
DEMO_OWNER_PASSWORD = "demo12345678"


def seed_demo_institute():
    conn = get_conn()
    cur = conn.cursor()
    try:
        cur.execute("SELECT id FROM institutes WHERE LOWER(email) = %s", (DEMO_OWNER_EMAIL,))
        if cur.fetchone():
        ...
        cur.execute(
            """INSERT INTO institutes (institute_name, full_name, email, password_hash, password_salt, created_at)
               VALUES (%s, %s, %s, %s, %s, %s) RETURNING id""",
            ("Sunrise International Academy", "Samarth Dave", DEMO_OWNER_EMAIL,
             hash_password(DEMO_OWNER_PASSWORD), "", now),
        )
        inst = cur.fetchone()[0]

        # ---- Branches ------------------------------------------------------
        branch_names = ["Main Campus — Andheri", "North Campus — Borivali", "South Campus — Bandra"]
        bids = []
        for bn in branch_names:
            ...
            bids.append(cur.fetchone()[0])
        main_b = bids[0]

        # ---- Staff users ---------------------------------------------------
        staff_rows = [
            ("Rajesh Iyer", "principal@ibex.in", "edit", "Principal",
             ["analytics", "assistant", "students", "teachers", "classrooms",
              "attendance", "syllabus", "timetables", "whatsapp",
              "seating", "invigilation", "results", "history",
              "inquiry", "fees"]),
            ("Meera Kulkarni", "accounts@ibex.in", "edit", "Accountant",
             ["fees", "inquiry", "whatsapp", "analytics"]),
            ("Anita Deshmukh", "teacher@ibex.in", "read_only", "Teacher",
             ["attendance", "syllabus", "timetables", "results", "history"]),
        ]
        for name, email, perm, desig, mods in staff_rows:
            cur.execute(
                """INSERT INTO staff_users (institute_id, full_name, email, password_hash, password_salt,
                                            permission, designation, module_access, created_at)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                (inst, name, email, hash_password(DEMO_OWNER_PASSWORD), "", perm, desig, json.dumps(mods), now),
            )

        # ---- Classrooms ----------------------------------------------------
        main_rooms = [
            ("LH-101", 60), ("LH-102", 60), ("LH-201", 45), ("LH-202", 45),
            ("LH-301", 30), ("Science Lab A", 30), ("Science Lab B", 30),
            ("Computer Lab", 40), ("Seminar Hall", 80), ("Auditorium", 200),
        ]
        for rn, cap in main_rooms:
            cur.execute(
                "INSERT INTO classrooms (branch_id, room_no, capacity, building) VALUES (%s, %s, %s, %s)",
                (main_b, rn, cap, "Main Block"),
            )
        for bn in bids[1:]:
            for rn, cap in [("A-101", 55), ("A-102", 45), ("B-201", 40), ("Lab-1", 30)]:
                cur.execute(
                    "INSERT INTO classrooms (branch_id, room_no, capacity, building) VALUES (%s, %s, %s, %s)",
                    (bn, rn, cap, "Academic Block"),
                )

        # ---- Teachers ------------------------------------------------------
        teachers = [
            ("Dr. Ramesh Kumar", "Physics", "+91 98200 11223"),
            ("Prof. Suresh Iyer", "Chemistry", "+91 98200 11224"),
            ("Dr. Anjali Sharma", "Mathematics", "+91 98200 11225"),
            ("Prof. Vikram Joshi", "Biology", "+91 98200 11226"),
            ("Ms. Priya Nair", "English", "+91 98200 11227"),
            ("Mr. Sanjay Patel", "Computer Science", "+91 98200 11228"),
            ("Mrs. Kavita Reddy", "Hindi", "+91 98200 11229"),
            ("Dr. Arjun Mehta", "History & Civics", "+91 98200 11230"),
        ]
        for n, s, c in teachers:
            cur.execute(
                "INSERT INTO teachers (branch_id, name, subject, contact_number, department) VALUES (%s, %s, %s, %s, %s)",
                (main_b, n, s, c, "Academics"),
            )

        # ---- Students ------------------------------------------------------
        batch_roster = {
            "JEE 2026 — Batch A": [
                ("Aarav Sharma", "+91 98765 10001"),
                ("Vivaan Patel", "+91 98765 10002"),
                ("Aditya Iyer", "+91 98765 10003"),
                ("Ananya Singh", "+91 98765 10004"),
                ("Diya Menon", "+91 98765 10005"),
                ("Rohan Kulkarni", "+91 98765 10006"),
            ],
            "JEE 2026 — Batch B": [
                ("Arjun Nair", "+91 98765 10011"),
                ("Reyansh Joshi", "+91 98765 10012"),
                ("Aadhya Bansal", "+91 98765 10013"),
                ("Sara Khan", "+91 98765 10014"),
                ("Kabir Malhotra", "+91 98765 10015"),
                ("Ira Saxena", "+91 98765 10016"),
            ],
            "NEET 2026 — Batch A": [
                ("Vihaan Reddy", "+91 98765 10021"),
                ("Sai Krishna", "+91 98765 10022"),
                ("Anika Mukherjee", "+91 98765 10023"),
                ("Navya Pillai", "+91 98765 10024"),
                ("Dhruv Chawla", "+91 98765 10025"),
                ("Myra Kapoor", "+91 98765 10026"),
            ],
            "NEET 2026 — Batch B": [
                ("Ayaan Verma", "+91 98765 10031"),
                ("Krishna Rao", "+91 98765 10032"),
                ("Kiara Desai", "+91 98765 10033"),
                ("Aisha Malik", "+91 98765 10034"),
                ("Yash Thakur", "+91 98765 10035"),
                ("Zara Sheikh", "+91 98765 10036"),
            ],
            "Foundation X": [
                ("Ishaan Gupta", "+91 98765 10041"),
                ("Riya Bhatt", "+91 98765 10042"),
                ("Tanvi Chopra", "+91 98765 10043"),
                ("Om Pandey", "+91 98765 10044"),
                ("Pihu Agarwal", "+91 98765 10045"),
                ("Vanya Sinha", "+91 98765 10046"),
            ],
        }
        all_students = []
        for batch, roster in batch_roster.items():
            prefix = {"JEE 2026 — Batch A": "JEEA", "JEE 2026 — Batch B": "JEEB",
                      "NEET 2026 — Batch A": "NEETA", "NEET 2026 — Batch B": "NEETB",
                      "Foundation X": "FND"}[batch]
            for i, (nm, contact) in enumerate(roster, 1):
                roll = f"{prefix}-{i:02d}"
                cur.execute(
                    """INSERT INTO students (branch_id, name, batch, status, roll_number, parent_contact)
                       VALUES (%s, %s, %s, %s, %s, %s)""",
                    (main_b, nm, batch, "Active", roll, contact),
                )
                all_students.append((nm, batch, roll))
        for bn in bids[1:]:
            for i, nm in enumerate(["Karan Joshi", "Neha Kapoor", "Rishi Bhatia", "Sneha Rao", "Manav Shah"], 1):
                cur.execute(
                    """INSERT INTO students (branch_id, name, batch, status, roll_number, parent_contact)
                       VALUES (%s, %s, %s, %s, %s, %s)""",
                    (bn, nm, "Foundation X", "Active", f"FND-{i:02d}", f"+91 98765 2000{i}"),
                )

        # ---- Syllabus ------------------------------------------------------
        syllabus = [
            ("Physics", "Kinematics & Laws of Motion", "Dr. Ramesh Kumar", 8, "2026-04-15"),
            ("Physics", "Work, Energy & Power", "Dr. Ramesh Kumar", 6, "2026-04-22"),
            ("Chemistry", "Atomic Structure", "Prof. Suresh Iyer", 7, "2026-04-18"),
            ("Chemistry", "Chemical Bonding", "Prof. Suresh Iyer", 8, "2026-04-25"),
            ("Mathematics", "Quadratic Equations", "Dr. Anjali Sharma", 6, "2026-04-16"),
            ("Mathematics", "Sequences & Series", "Dr. Anjali Sharma", 7, "2026-04-23"),
            ("Biology", "Cell Structure & Function", "Prof. Vikram Joshi", 8, "2026-04-17"),
            ("Biology", "Human Physiology", "Prof. Vikram Joshi", 10, "2026-04-24"),
            ("English", "Reading Comprehension", "Ms. Priya Nair", 5, "2026-04-19"),
            ("English", "Creative Writing", "Ms. Priya Nair", 4, "2026-04-26"),
            ("Computer Science", "Python Fundamentals", "Mr. Sanjay Patel", 8, "2026-04-20"),
            ("Computer Science", "Data Structures Intro", "Mr. Sanjay Patel", 7, "2026-04-27"),
            ("Hindi", "गद्य खंड", "Mrs. Kavita Reddy", 5, "2026-04-21"),
            ("History & Civics", "Indian Constitution", "Dr. Arjun Mehta", 6, "2026-04-28"),
        ]
        for s, t, tn, nl, ld in syllabus:
            cur.execute(
                """INSERT INTO syllabus (branch_id, subject, topic, teacher_name, num_lectures, lecture_date)
                   VALUES (%s, %s, %s, %s, %s, %s)""",
                (main_b, s, t, tn, nl, ld),
            )
        ...
        tt_batches = [
            ("JEE 2026 — Batch A", ["Physics", "Chemistry", "Mathematics", "English", "Computer Science"], "LH-101"),
            ("JEE 2026 — Batch B", ["Mathematics", "Physics", "Biology", "Hindi", "Chemistry"], "LH-102"),
            ("NEET 2026 — Batch A", ["Biology", "Chemistry", "Physics", "English", "Mathematics"], "LH-201"),
        ]
        teacher_by_subject = {t[1]: t[0] for t in teachers}
        for batch_name, subjects, room in tt_batches:
            for di, day in enumerate(days):
                for si, (time_slot, lec_num) in enumerate(time_slots[:4]):
                    subj = subjects[(di + si) % len(subjects)]
                    teacher = teacher_by_subject.get(subj, "Dr. Ramesh Kumar")
                    cur.execute(
                        """INSERT INTO timetables_slots
                           (branch_id, batch_name, day, time_slot, lecture_number, subject, teacher, room)
                           VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
                        (main_b, batch_name, day, time_slot, lec_num, subj, teacher, room),
                    )
            timings_json = json.dumps([{"lecture_number": ln, "time_slot": ts} for ts, ln in time_slots])
            teachers_cfg = json.dumps([
                {"name": t[0], "subject": t[1], "lectures_per_week": 3, "unavailable_days": []}
                for t in teachers[:5]
            ])
            cur.execute(
                """INSERT INTO timetable_configs
                   ...
                   VALUES (%s, %s, %s, %s, %s)""",
                (main_b, batch_name, timings_json, teachers_cfg, now),
            )

        # ---- Seating plans -------------------------------------------------
        seating_pool = [
            {"id": i + 1, "name": nm, "batch": batch, "roll_number": roll}
            for i, (nm, batch, roll) in enumerate(all_students)
        ]

        def build_assignments(students_pool, rows, cols):
            pool = list(students_pool)
            random.shuffle(pool)
            assignments = []
            idx = 0
            for r in range(rows):
                for c in range(cols):
                    ...
                    assignments.append({
                        ...
                        "batch": s["batch"], "roll_number": s["roll_number"],
                    })
            return assignments

        assignments_a = build_assignments(seating_pool, 4, 5)
        cur.execute(
            """INSERT INTO exam_seatings
               (branch_id, exam_date, room_number, rows, columns, assignments_json, created_at)
               VALUES (%s, %s, %s, %s, %s, %s, %s)""",
            (main_b, "2026-05-10", "LH-101", 4, 5, json.dumps(assignments_a), now),
        )
        assignments_b = build_assignments(seating_pool, 4, 5)
        cur.execute(
            """INSERT INTO exam_seatings
               (branch_id, exam_date, room_number, rows, columns, assignments_json, created_at)
               VALUES (%s, %s, %s, %s, %s, %s, %s)""",
            (main_b, "2026-05-12", "LH-102", 4, 5, json.dumps(assignments_b), now),
        )

        # ---- Invigilation --------------------------------------------------
        invig = [
            ("Dr. Ramesh Kumar", "2026-05-10", "LH-101"),
            ("Prof. Suresh Iyer", "2026-05-10", "LH-102"),
            ("Dr. Anjali Sharma", "2026-05-11", "LH-201"),
            ("Prof. Vikram Joshi", "2026-05-12", "LH-101"),
            ("Ms. Priya Nair", "2026-05-12", "LH-102"),
        ]
        for tn, ed, rm in invig:
            cur.execute(
                "INSERT INTO invigilation (branch_id, teacher_name, exam_date, room) VALUES (%s, %s, %s, %s)",
                (main_b, tn, ed, rm),
            )

        # ---- Fees ----------------------------------------------------------
        fee_rows = [
            ("Aarav Sharma", 75000, "Paid", "2026-04-05", "UTR20260405001"),
            ("Vivaan Patel", 75000, "Paid", "2026-04-05", "UTR20260405002"),
            ("Aditya Iyer", 75000, "Pending", "2026-04-10", None),
            ("Ananya Singh", 75000, "Paid", "2026-04-08", "UTR20260408003"),
            ("Diya Menon", 75000, "Pending", "2026-04-15", None),
            ("Rohan Kulkarni", 65000, "Paid", "2026-04-12", "UTR20260412004"),
            ("Arjun Nair", 75000, "Paid", "2026-04-06", "UTR20260406005"),
            ("Reyansh Joshi", 75000, "Pending", "2026-04-20", None),
            ("Aadhya Bansal", 75000, "Paid", "2026-04-09", "UTR20260409006"),
            ("Sara Khan", 75000, "Paid", "2026-04-11", "UTR20260411007"),
            ("Kabir Malhotra", 75000, "Pending", "2026-04-25", None),
            ("Ira Saxena", 75000, "Paid", "2026-04-14", "UTR20260414008"),
            ("Vihaan Reddy", 80000, "Paid", "2026-04-07", "UTR20260407009"),
            ("Sai Krishna", 80000, "Pending", "2026-04-22", None),
            ("Anika Mukherjee", 80000, "Paid", "2026-04-13", "UTR20260413010"),
        ]
        for sn, amt, st, dd, utr in fee_rows:
            if st == "Paid":
                cur.execute(
                    """INSERT INTO fees (branch_id, student_name, amount_inr, status, due_date, utr_reference, paid_at)
                       VALUES (%s, %s, %s, %s, %s, %s, NOW())""",
                    (main_b, sn, amt, st, dd, utr),
                )
            else:
                cur.execute(
                    """INSERT INTO fees (branch_id, student_name, amount_inr, status, due_date, utr_reference)
                       VALUES (%s, %s, %s, %s, %s, %s)""",
                    (main_b, sn, amt, st, dd, utr),
                )

        # ---- Exam results --------------------------------------------------
        for nm, batch, roll in all_students[:18]:
            marks = random.randint(45, 98)
            cur.execute(
                """INSERT INTO exam_results
                   (branch_id, batch_name, subjects, topics, exam_date, overall_marks,
                    student_name, roll_number, marks)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                (main_b, batch, "Physics, Chemistry, Mathematics",
                 "Kinematics, Bonding, Quadratics", "2026-04-20", 100, nm, roll, marks),
            )
        for nm, batch, roll in all_students[:12]:
            ...
            cur.execute(
                """INSERT INTO exam_results
                   (branch_id, batch_name, subjects, topics, exam_date, overall_marks,
                    student_name, roll_number, marks)
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                (main_b, batch, "Biology, Chemistry, Physics",
                 "Cell Structure, Organic, Optics", "2026-04-22", 100, nm, roll, marks),
            )

        # ---- Exam history --------------------------------------------------
        hist = [
            ("Physics", "Kinematics & Laws of Motion", "JEE 2026 — Batch A", "2026-04-20"),
            ("Chemistry", "Atomic Structure", "JEE 2026 — Batch A", "2026-04-20"),
            ("Mathematics", "Quadratic Equations", "JEE 2026 — Batch A", "2026-04-20"),
            ("Biology", "Cell Structure & Function", "NEET 2026 — Batch A", "2026-04-22"),
            ("Chemistry", "Chemical Bonding", "NEET 2026 — Batch A", "2026-04-22"),
            ("Physics", "Work, Energy & Power", "JEE 2026 — Batch B", "2026-04-23"),
        ]
        for s, t, b, d in hist:
            cur.execute(
                "INSERT INTO exam_history (branch_id, subject, topic, batch_name, exam_date) VALUES (%s, %s, %s, %s)",
                (main_b, s, t, b, d),
            )

        # ---- Journal entries ----------------------------------------------
        journal = [
            ("Great start to the new session! Attendance has been strong across the JEE batches. "
             "Need to focus on the NEET batches' Physics scores going forward."),
            ("Met with the accounts team today — pending fees are concentrated in a few families. "
             "Sending WhatsApp reminders this week."),
            ("The new Science Lab B is fully operational. Teachers are happy with the upgrade. "
             "Next up: expanding the Computer Lab capacity."),
        ]
        for (content,) in journal:
            cur.execute(
                """INSERT INTO journal_entries
                   (institute_id, branch_id, author_user_id, author_name, content, created_at)
                   VALUES (%s, %s, %s, %s, %s, %s)""",
                (inst, main_b, inst, "Samarth Dave", content, now),
            )

        # ---- Inquiries -----------------------------------------------------
        inquiries = [
            ("Rakesh Agarwal", "+91 99887 76655", "Walk-in", "JEE 2026 — Batch A", "New", "2026-05-01"),
            ("Sunita Rao", "+91 99887 76656", "Referral", "NEET 2026 — Batch A", "Contacted", "2026-05-02"),
            ("Vijay Kumar", "+91 99887 76657", "Ad", "Foundation X", "New", "2026-05-03"),
            ("Priyanka Shetty", "+91 99887 76658", "Walk-in", "JEE 2026 — Batch B", "Converted", "2026-04-28"),
            ("Arun Bhatt", "+91 99887 76659", "Ad", "NEET 2026 — Batch B", "New", "2026-05-04"),
        ]
        for n, p, src, ii, st, fud in inquiries:
            cur.execute(
                """INSERT INTO inquiries (branch_id, name, phone, source, interested_in, status, follow_up_date)
                   VALUES (%s, %s, %s, %s, %s, %s, %s)""",
                (main_b, n, p, src, ii, st, fud),
            )

        # ---- Audit log -----------------------------------------------------
        audit_rows = [
            (inst, main_b, "CREATE_INSTITUTE", {"institute_name": "Sunrise International Academy"}),
            (inst, main_b, "CREATE_BRANCH", {"name": "Main Campus — Andheri"}),
            (inst, main_b, "CREATE_USER", {"full_name": "Rajesh Iyer", "permission": "edit"}),
            (inst, main_b, "GENERATE_TIMETABLE", {"batch_name": "JEE 2026 — Batch A", "slots": 20}),
            (inst, main_b, "GENERATE_SEATING", {"exam_date": "2026-05-10", "room_number": "LH-101"}),
            (inst, main_b, "WHATSAPP_BROADCAST", {"sent": 48, "message": "Institute will remain closed on 1st May for Labour Day."}),
        ]
        for iid, bid, atype, payload in audit_rows:
            cur.execute(
                """INSERT INTO audit_log (timestamp, user_id, branch_id, action_type, before_after_payload)
                   VALUES (NOW(), %s, %s, %s, %s::jsonb)""",
                (iid, bid, atype, json.dumps({"before": None, "after": payload})),
            )

        conn.commit()
        print(f"[demo] Seeded 'Sunrise International Academy' — login: {DEMO_OWNER_EMAIL} / {DEMO_OWNER_PASSWORD}")
    except Exception as exc:
        conn.rollback()
...
# Kick off seeding at import time (idempotent, so safe on every boot).
try:
    seed_demo_institute()
except Exception as _seed_err:
    print(f"[demo] Seeder skipped: {_seed_err}")
